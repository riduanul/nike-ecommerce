                    ...
                    <Splide options={slideOptions}>
                        {
                            news.map((val, i)=> (
                                <SplideSlide key={i} className="mb-0.5">
                                       <div className='relative grid items-center gap-4 shadow shadow-slate-200 pb-2 rounded-lg ring-1 ring-slate-200 '>
                                        ...
                                        <div className='grid items-center justify-items-center px-4'>
                                            <h1 className='text-base lg:text-sm font-semibold'>{val.title}</h1>
                                            <p className='text-sm text-justify lg:text-xs'>{truncate(val.text, {"length": 175})}</p>
                                        </div>
                                        <div className='flex items-center justify-center px-4 w-full'>
                                           <a href={val.url} target={"_blank"} className='w-full bg-gradient-to-b from-slate-900 to-black shadow-md shadow-black text-center text-slate-100 py-1.5 button-theme'>{val.btn}</a> 
                                        </div>
                                    </div>
                                </SplideSlide>
                            ))
